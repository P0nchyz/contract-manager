...
<template>
  <UDashboardPanel id="lognote-new">
    ...
    <template #body>
      <div class="max-w-2xl">
        <!-- Lock notice -->
        <UAlert color="neutral" variant="soft" icon="i-lucide-lock" :title="L.lockedNotice" class="mb-4" />

        <UCard>
          <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
            <UFormField :label="L.fields.title" name="title">
              <UInput v-model="state.title" class="w-full" :placeholder="L.fields.titlePlaceholder" />
            </UFormField>
            ...
            <UFormField :label="L.fields.body" name="body">
              <UTextarea v-model="state.body" :rows="6" class="w-full" :placeholder="L.fields.bodyPlaceholder" />
            </UFormField>
            ...
            <div class="flex justify-end gap-3 border-t border-default pt-4">
              <UButton color="neutral" variant="ghost" :to="`/contracts/${contractId}/logbook`">
                {{ S.common.cancel }}
              </UButton>
